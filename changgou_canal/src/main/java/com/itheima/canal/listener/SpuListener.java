...
public class SpuListener {
    ...
    public void spuUp(CanalEntry.EventType eventType, CanalEntry.RowData rowData) {
        ...
        if ("1".equals(newMap.get("is_marketable")) && "0".equals(oldMap.get("is_marketable"))) {
            System.err.println("上架消息已发出");
            rabbitTemplate.convertAndSend(RabbitMQConfig.GOODS_UP_EXCHANGE, "", newMap.get("id")); //发送到mq商品上架交换器上
        }
        if ("0".equals(newMap.get("is_marketable")) && "1".equals(oldMap.get("is_marketable"))) {
            System.err.println("下架消息已发出");
